...
impl MyHashMap {
    ...
    fn put(&mut self, key: i32, value: i32) {
        if let Some(idx) = self.key.iter().position(|&r| r == key) {
    ...
    fn remove(&mut self, key: i32) {
        if let Some(idx) = self.key.iter().position(|&r| r == key) {
            self.key.remove(idx as usize);
            self.value.remove(idx as usize);
        }
    }
}
...
mod tests {
    ...
    fn test_706() {
        ...
        obj.put(2, 3);
        obj.put(4, 5);
        println!("{:#?}", obj); // derived trait Debug (like decorator in python)

        let ret_2: i32 = obj.get(2);
        println!("{}", ret_2);

        let ret_0: i32 = obj.get(0);
        println!("{}", ret_0);
